         ...
         )
{
     ...
     */
    if (nlhs < 1)
        mexErrMsgTxt("igraphcut requires an output argument.");
    
    if (nrhs < 2)
        mexErrMsgTxt("igraphcut requires at least two input arguments.");
     ...
     */
    float K = (int) mxGetScalar(prhs[1]);
    int min_size = (int) mxGetScalar(prhs[2]);
	...
	/*
     * do the graph cuts algorithm
     */
    int num_ccs; 
    
    //printf("here we go %f %d %f\n", K, min_size, sigma);
    image<int> *seg = segment_image(r, g, b, sigma, K, min_size, &num_ccs); 
